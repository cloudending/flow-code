...
void MotionToColor(FloatImage motim, ByteImage &colim, float maxmotion)
{
    ...
    int width = motim.GetWidth(), height = motim.GetHeight();
	int channels = motim.GetChannels();
    //colim.ReAllocate(CShape(width, height, 3));
	colim.Allocate(width, height, 3);
    int x, y;
    // determine motion range:
    float maxx = -999, maxy = -999;
    float minx =  999, miny =  999;
    float maxrad = -1;
	float* motimdata = motim.GetData();
    for (y = 0; y < height; y++) {
	for (x = 0; x < width; x++) {
	    //float fx = motim.Pixel(x, y, 0);
	    //float fy = motim.Pixel(x, y, 1);
		float fx = motimdata[(y*width+x)*channels];
		float fy = motimdata[(y*width+x)*channels+1];
		//printf("%f %f\n",fx,fy);
	...
	for (x = 0; x < width; x++) {
		...
		uchar *pix = &colim.GetData()[(y*width+x)*3];
